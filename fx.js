...
const QUEUE = 'ncdc-report';

/**
 * @description This function opens a connection to the RabbitMQ server,
 * @author Olabosinde Oladipo Olabosindeoladipo@gmail.com
 */
exports.openAMQP =  amqplib.connect('amqp://localhost');
...
exports.requestForUrl = async () => {

    // Prompt user for URL
    url = 'https://covid19.ncdc.gov.ng/'; // Choose input URL or the default
    exports.url = url;
    return exports.url;
}
...
exports.requestForIdentifier = async (l) => {

    // Prompt user for the identifier of the table on the DOM
    identifier = '#custom1'; // Choose input identifier or the default
...
exports.extractTableHTMLtoObject = async ($, identifier) => {
...
exports.generateMutation = (mainObject, url, CURRENT_DIR, MUTATIONS_DIR) => {
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            if (files.filter(fn => fn.startsWith(exports.extractHostname(url) + '_')).length > 0) {
                ...
                // Save file as current mutation

                // Add date to it
                var d = new Date();
                var d_str = d.getDate() + '/' + ((d.getMonth() + 1) <= 9 ? ('0' + (d.getMonth() + 1)) : (d.getMonth() + 1)) + '/' + d.getFullYear();
                ...
                fs.writeFile(`${MUTATIONS_DIR}/current_mutation` , JSON.stringify(mutation_copy), function (err) {
                    ...
                    subject.next('Saved current mutation to directory ...');
                });
                    ...
                    rimraf(existingCurrentFile).then(() => {
                        ...
                        fs.promises.writeFile(currentFile, JSON.stringify(mainObject), 'utf8').then(
                            () => {
                                ...
                                exports.publishToQueue(exports.openAMQP, QUEUE, JSON.stringify(mutation_copy) )
                            }
...
exports.publishToQueue = (rmq, queue, msg)=> {
    subject.next('Publishing to queue...');
    // Publisher
    rmq.then(function (conn) {
        return conn.createChannel();
    }).then(function (ch) {
        return ch.assertQueue(queue).then(function (ok) {
            let s = ch.sendToQueue(queue, Buffer.from(msg));
            subject.next('Published to queue successfully');
            return s;
        });
    }).catch(console.warn);
}